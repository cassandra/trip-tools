...
class TripCreateModalViewTests(TestCase):
    ...
    def setUp(self):
        ...
        self.trips_create_url = reverse('trips_trip_create')
...
class TripOverviewViewTests(TestCase):
    ...
    def setUp(self):
        self.client = Client()
        self.trip_overview_url = reverse('trips_trip_overview', kwargs={'trip_uuid': self.trip.uuid})

    def test_trip_overview_requires_authentication(self):
        """Test that trip overview requires authentication."""
        response = self.client.get(self.trip_overview_url)
        self.assertEqual(response.status_code, 302)
        self.assertIn('/user/signin', response.url)

    def test_trip_overview_displays(self):
        """Test that trip overview displays correctly."""
        self.client.force_login(self.user)
        response = self.client.get(self.trip_overview_url)
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, 'Test Trip')

    def test_trip_overview_sets_session(self):
        """Test that visiting trip overview sets the trip in session."""
        self.client.force_login(self.user)
        response = self.client.get(self.trip_overview_url)
        self.assertEqual(response.status_code, 200)
    ...
    def test_trip_overview_only_shows_user_trips(self):
        """Test that trip overview only shows trips for the logged-in user."""
        ...
        self.client.force_login(self.user)
        other_trip_url = reverse('trips_trip_overview', kwargs = { 'trip_uuid': other_trip.uuid })
        response = self.client.get(other_trip_url)

        # Should return a 404 since user doesn't own the trip
        self.assertEqual(response.status_code, 404)

    def test_trip_overview_includes_trip_page_context(self):
        """Test that trip overview includes trip_page context with proper active_page."""
        self.client.force_login(self.user)
        response = self.client.get(self.trip_overview_url)
    ...
    def test_trip_overview_nonexistent_trip(self):
        """Test that requesting nonexistent trip returns 404."""
        self.client.force_login(self.user)
        url = reverse('trips_trip_overview', kwargs={'trip_uuid': '32653d87-9b24-4c8f-adfb-8ab876418072'})
        response = self.client.get(url)
        self.assertEqual(response.status_code, 404)


class TripsHOmeViewTests(TestCase):
    """Tests for the trips_home view that shows all trips with categorization."""
    ...
    def setUp(self):
        ...
        self.trips_home_url = reverse('trips_home')

    def test_trips_home_requires_authentication(self):
        """Test that trips_home redirects unauthenticated users."""
        response = self.client.get(self.trips_home_url)
        self.assertEqual(response.status_code, 302)
        self.assertIn('/user/signin', response.url)

    def test_trips_home_displays_for_authenticated_user(self):
        """Test that trips_home displays for authenticated users."""
        self.client.force_login(self.user)
        response = self.client.get(self.trips_home_url)
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'trips/pages/trips_home.html')

    def test_trips_home_shows_upcoming_trips(self):
        """Test that trips_home displays upcoming trips."""
        self.client.force_login(self.user)
    ...
    def test_trips_home_shows_past_trips(self):
        """Test that trips_home displays past trips separately."""
        self.client.force_login(self.user)
    ...
    def test_trips_home_only_shows_user_trips(self):
        """Test that trips_home only shows trips for the logged-in user."""
        other_user = User.objects.create_user(
            email='other@example.com',
            password='testpass123'
        )
    ...
    def test_trips_home_categorizes_trips_by_permission_and_status(self):
        """Test that trips_home correctly categorizes trips by permission level and status."""
        self.client.force_login(self.user)
