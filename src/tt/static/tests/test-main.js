...
(function() {
    ...
    QUnit.module('Tt.generateUniqueId', function(hooks) {
        let originalDateNow;
        let mockTime;
        ...
        QUnit.test('generates different IDs on different calls', function(assert) {
            ...
            const id1 = Tt.generateUniqueId();
            const id2 = Tt.generateUniqueId();
    ...
    QUnit.module('Hi Cookie Functions', function(hooks) {
        ...
        QUnit.test('setCookie sets cookie with correct format', function(assert) {
            const result = Tt.setCookie('testName', 'testValue', 7);
        ...
        QUnit.test('setCookie handles URL encoding', function(assert) {
            Tt.setCookie('test', 'value with spaces & symbols');
        ...
        QUnit.test('getCookie retrieves set cookie', function(assert) {
            Tt.setCookie('retrieveTest', 'retrieveValue');
            
            const value = Tt.getCookie('retrieveTest');
        ...
        QUnit.test('getCookie handles URL decoding', function(assert) {
            ...
            const value = Tt.getCookie('encoded');
        ...
        QUnit.test('getCookie returns null for non-existent cookie', function(assert) {
            const value = Tt.getCookie('nonExistent');
        ...
        QUnit.test('getCookie handles multiple cookies', function(assert) {
            ...
            assert.equal(Tt.getCookie('first'), 'value1', 'Gets first cookie');
            assert.equal(Tt.getCookie('second'), 'value2', 'Gets middle cookie');
            assert.equal(Tt.getCookie('third'), 'value3', 'Gets last cookie');
            assert.strictEqual(Tt.getCookie('fourth'), null, 'Non-existent returns null');
        });
        ...
        QUnit.test('getCookie handles partial name matches correctly', function(assert) {
            ...
            assert.equal(Tt.getCookie('test'), 'value1', 'Exact match returns correct value');
            assert.equal(Tt.getCookie('testLonger'), 'value2', 'Longer name with same prefix works');
            assert.equal(Tt.getCookie('pretest'), 'value3', 'Name as suffix works');
        });
        
        QUnit.test('cookie functions handle edge cases', function(assert) {
            // Empty value
            Tt.setCookie('empty', '');
            assert.equal(Tt.getCookie('empty'), '', 'Empty cookie value handled');
            
            // Special characters in name (encoded)
            Tt.setCookie('special-name_123', 'specialValue');
            assert.equal(Tt.getCookie('special-name_123'), 'specialValue', 'Special chars in name handled');
            
            // Cookie with equals in value
            mockCookie = 'equation=a%3Db%2Bc';
            assert.equal(Tt.getCookie('equation'), 'a=b+c', 'Equals signs in value handled');
        });
    });
